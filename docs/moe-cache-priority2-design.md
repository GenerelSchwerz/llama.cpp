...
Status: proposed design, not implemented or performance-qualified. Prepared on 2026-09-20 for `moe-cache-multi-gpu`. Initial inspection used `edb542798b6577451722281fb0288f944f47a1e1`; during the task HEAD advanced to `d50dcda11a0db15635f825172b4591a6408e4a70`. The complete intervening diff was reviewed: six lines in `common/common.cpp` log the positive host-pin budget at info level, with no placement/accounting change. This design covers that final HEAD; the recorded Qwen measurements remain specifically `edb542798`. This task inspected code, documents, existing artifacts, and primary references; it did not build, test, benchmark, commit, push, or change implementation files.
