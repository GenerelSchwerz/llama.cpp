#!/usr/bin/env bash
# run_synth.sh <tag> <port> <n_prefill_tokens> <n_predict> <extra server args...>
# Same lifecycle as run_one.sh but sends a raw /completion request with an
# explicit repeated-token-ID prompt array, for clean prefill-throughput
# measurement independent of tokenizer/content effects.
set -u
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUTDIR="$SCRIPT_DIR/results"
mkdir -p "$OUTDIR"

TAG="$1"; PORT="$2"; NPREFILL="$3"; NPREDICT="$4"; shift 4
EXTRA_ARGS=("$@")
...
echo "=== $TAG (synthetic, prefill=$NPREFILL, predict=$NPREDICT) ===" | tee -a "$OUTDIR/SUMMARY.txt"
...
if [ "$UP" -ne 1 ]; then
  ...
  kill "$SPID" 2>/dev/null; wait "$SPID" 2>/dev/null; exit 1
fi
...
import json
n = $NPREFILL
prompt = [(100 + (i % 50)) for i in range(n)]
print(json.dumps({'prompt': prompt, 'n_predict': $NPREDICT, 'temperature': 0, 'seed': 1234, 'cache_prompt': False, 'stream': False}))
")

curl -sS -o "$RESP" -H 'Content-Type: application/json' --data-binary "$PAYLOAD" "http://127.0.0.1:${PORT}/completion"
