...
- `moe-grouped-owner`: owner, physical device, PCI identity, generation, grouped/legacy dispatch counts, direct/capture/replay dispatch counts, ready/submitted/completed counts and preparation/completion errors. Main, separate-draft and MTP dispatches are attributed independently. For device-direct grouped execution it also reports route and unique accesses, resident hits and demand misses, current populated slots, expert payload allocation, slot/fixed auxiliary, planner metadata and pinned host-staging bytes, vacant/replacement/post-invalidation refills, refill bytes by source path (direct registered, pageable staged, mapped, device or adopted prepack), early-router proposed/copied/used/wrong/late/dropped bytes, demand-materialized bytes, and reset counts by cause. Shutdown retains owner totals even after the semantic table is cleared.
- `moe-grouped-paths`, `moe-grouped-resources`, and `moe-grouped-prefetch`: process aggregates for the corresponding owner fields. `h2d_bytes` is gathered miss-bank payload, not necessarily physical host-to-device traffic; `vacant_fills` means a slot first populated in the current resource generation; `source_prepack_bytes` and `prefetch_used_bytes` are adopted prefetch payload.
...
Refill and source-byte fields are interval counters; populated slots and payload are a point-in-time sample of live resources. A `0/0` occupancy sample on folded or retired telemetry means no live resource was available to sample, not proof that an existing cache was empty. Reset-only idle replacements do not create process-wide activity records, but reset causes remain attached to direct samples and folded intervals that contain execution activity. These diagnostics allocate their additional device counters and invalidation bitmap only while `--experimental-logs` is enabled.

`host_staging_bytes` samples the owner budget's live staging allocation, including grouped materialization, legacy per-bank scratch, grouped-host-staged per-bank scratch, and optional prepack growth. Teardown labels any final prepack scalar line as a remaining interval and its per-rank line as lifetime data so interval resets cannot be mistaken for lifetime totals.

Grouped-host-staged execution reports admission and operation counts through `host_staged_calls`, `host_staged_ops`, and `host_staged_split_ops`. Its authoritative per-bank residency, hit, miss, eviction, and transfer counters remain in `moe-cache-phase`; the grouped `route_accesses`, `cache_hits`, and `cache_misses` fields are device-planner counters and deliberately do not duplicate the host-staged cache totals.
...
The two-layer fixture uses the production graph builder and real scheduler, shared immutable host sources, and separate CUDA owners. It covers fused Q4_0, separate Q4_K and ungated layouts; direct execution, capture and replay; changing routes; a 512 KiB bounded source/staging budget; candidate replacement on one owner; owner teardown; and injected failure on the second layer. It now also covers ordinary/cache and cache/ordinary placement with real ordinary CUDA buffers, all-ordinary and all-cache boundaries, and full-slot residency. Every positive owner must complete exactly eight grouped executions during the main comparison, with zero legacy dispatches, against an independent all-ordinary exact-output oracle. Ordinary groups must allocate no cache resource and report no cache traffic. Separate bidirectional exact-copy checks require both CUDA copy interfaces to decline when peer access is unsupported, then exercise the generic synchronous and asynchronous fallback APIs.
...
Production-like named fixtures also verify route-ID publication across mixed boundaries. Fused top-k publishes only when the next active MMID consumes the same IDs from a cached bank. The retained slot belongs to the producer tensor, storage address, and stream; dispatch freshness and abandoned-publication state prevent stale scratch aliases. Rebuilt graph UIDs reuse that slot, and the table remains bounded at 512 entries. Named mixed and all-cache cases require zero ID-copy synchronizations while fusion is enabled. Fusion-disabled and exhausted-table cases retain the ordered copy fallback.
