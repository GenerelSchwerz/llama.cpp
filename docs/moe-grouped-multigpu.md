...
- `moe-grouped-owner`: owner, physical device, PCI identity, generation, grouped/legacy dispatch counts, direct/capture/replay dispatch counts, ready/submitted/completed counts and preparation/completion errors. It also reports current populated slots and payload capacity, vacant/replacement/post-invalidation refills, refill bytes by source path (direct registered, pageable staged, mapped, device or adopted prepack), and reset counts by cause. Shutdown retains owner totals even after the semantic table is cleared.
