...
- The current auxiliary-shadow follow-up has local synthetic coverage for CUDA-unregistered NVFP4 scales, including exact output and graph replay. This is not physical multi-GPU or general pageable-auxiliary validation; the current dirty follow-up must not inherit the earlier physical validation claims below.
