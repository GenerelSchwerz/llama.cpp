...
| `--kv-gpu-window N` | `LLAMA_ARG_KV_GPU_WINDOW` | `0` | Experimental CUDA path for Q8_0/Q8_0 standard KV: keeps a same-format compact suffix of the newest `N` rows per layer on the device and excludes those rows from mapped pinned-host body reads. The complete Q8_0 host body remains authoritative, so this reduces repeated transfer rather than host allocation. Requires `256 <= N < n_ctx`, explicit Flash Attention and operation offload, one sequence, a default target context with non-SWA standard attention, `--no-kv-offload --kv-cpu-pinned`, `--kv-gpu-layers 0`, no tensor split, and no KVarN or precision tail. Every attention layer must resolve to the same CUDA registry device 0; unsupported or cross-device placement fails closed. The first transport-correctness milestone still sizes its temporary packed body for the complete `kv_size`; reducing that capacity to the cold region is a separate follow-up milestone. |
