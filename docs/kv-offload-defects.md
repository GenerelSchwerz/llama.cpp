...
**D7 blocks D5.** The KVarN build does not compile on this base, so D5's numbers
cannot be regenerated by anyone until D7 is fixed. That is why D5 was carried
forward rather than re-measured, and it is a stronger statement than "not
re-run": nobody can re-run it as the tree stands.
...
**This defect cannot be observed at all without first fixing
[D7](#d7-ggml_cuda_kvarnon-does-not-compile)**, since `GGML_CUDA_KVARN=ON` does
...
**Status: CONFIRMED on `946c1e5b6`, and the fix verified to build.**
**Owner: BeeLlama.** Severity: the configuration `CLAUDE.md` documents cannot be
built at all. Blocks D5 and every roadmap item that touches KVarN.
...
exit 0 and zero errors; without it, 82. The patch is included as a reference,
not as a proposal -- per the roadmap it should land as its own change.
