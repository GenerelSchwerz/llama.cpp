...
public:
    static std::unique_ptr<llama_staged_inputs> create(const llama_model & model, ggml_backend_t backend);
    ~llama_staged_inputs();
...
private:
    explicit llama_staged_inputs(const llama_model & model) : model(model) {}
