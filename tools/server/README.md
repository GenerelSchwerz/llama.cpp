...
| `--spec-draft-rs-planes, --spec-mtp-rs-planes N` | total target recurrent-state planes for draft-mtp or draft-dflash, including the current state (default: 0, use spec-draft-n-max + 1); see [capped recurrent planes](../../docs/speculative.md#capped-recurrent-planes)<br/>(env: LLAMA_ARG_SPEC_DRAFT_RS_PLANES) |
| `--spec-draft-rs-planes-lead N` | capped recurrent planes that keep the states after the first draft tokens; the others keep the newest states (default: -1, auto: spec-draft-rs-planes - 2 for draft-dflash, 0 for draft-mtp)<br/>(env: LLAMA_ARG_SPEC_DRAFT_RS_PLANES_LEAD) |
