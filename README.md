...
| `--moe-expert-cache-host-pinned-mb N` or `LLAMA_ARG_MOE_EXPERT_CACHE_HOST_PINNED_MB=N` | Omitted preserves existing defaults; explicit `0` is rejected | Experimental model-wide MiB cap on cache-owned CUDA source registrations and staging/control pins, for mmap and non-mmap loading. Requires a positive expert cache size. |
| `llama_model_params::moe_expert_cache_host_pinned_size` | `SIZE_MAX` from `llama_model_default_params()` preserves defaults | Library equivalent of the host pin budget, in bytes. |
| `--moe-expert-cache-l2-pinned-mb N` or `LLAMA_ARG_MOE_EXPERT_CACHE_L2_PINNED_MB=N` | Deprecated alias; explicit `0` now rejects staging | Uses the new host-budget semantics. Supplying both old and new names, including environment variables, is an error. |
...
With no explicit host budget, cold weights use pinned host memory when possible. If pinned allocation fails or `GGML_CUDA_NO_PINNED` is set, allocation falls back to regular CPU memory. Memory-mapped models page directly from their mapped source. These omitted-option defaults are unchanged.

With an explicit host budget, weights keep pageable or mmap backing, selected source ranges are registered in place, and grouped GPU misses use bounded pinned staging where direct registration is unavailable. The budget is shared by contexts using that model, not multiplied per context. A too-small budget or `GGML_CUDA_NO_PINNED` produces an error; this mode never silently exceeds its cap or requests CPU expert execution. It does not cap total RAM or unrelated backend/driver allocations. See the [experimental partial-pinning guide](docs/development/moe-partial-pinning.md) for zero/alias semantics, diagnostics, performance tradeoffs, and validation limits.
...
The experimental `ggml_backend_cuda_moe_bounded_buffer_type()` creates a refcounted host-budget owner. Retain its initial reference until all uses of the buffer type end, then release it with `ggml_backend_cuda_moe_bounded_buffer_type_free()`. Normal bounded buffers and staging allocations retain it; zero-sized generic buffers do not. The model loader handles this lifetime automatically. The legacy low-level L2 API remains for compatibility; the CLI no longer configures that separate cache.
