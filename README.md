...
The primary low-level placement APIs are `ggml_backend_cuda_moe_cached_buffer_type()`, `ggml_backend_buft_is_cuda_moe_cached()`, and `ggml_backend_cuda_moe_cached_buffer_from_host_ptr()`. The last function wraps a borrowed host range using the supplied buffer type. Bounded sources use `ggml_backend_cuda_moe_cached_bounded_buffer_type()` and one post-load `ggml_backend_cuda_moe_cached_configure_sources()` call; release the factory reference with `ggml_backend_cuda_moe_cached_free_buffer_type()` after buffer creation. Buffers and active cache resources retain the budget owner. `ggml_backend_cuda_moe_set_debug_mm()` / `ggml_backend_cuda_moe_get_debug_mm()` control detailed logging, and `ggml_backend_cuda_moe_log_and_reset_stats()` reports and clears aggregate counters.
