...
bool ggml_gated_delta_net_validate(const struct ggml_tensor * tensor) {
    ...
    if (tensor->type != GGML_TYPE_F32 || tensor->ne[0] != (int64_t) output_width ||
        tensor->ne[1] != (int64_t) output_rows || tensor->ne[2] != 1 || tensor->ne[3] != 1) {
        return false;
    }
...
size_t ggml_quantize_chunk(
    ...
    switch (type) {
        ...
        case GGML_TYPE_Q2_0S:   result = quantize_q2_0s  (src + start, (char *) dst + start_row * row_size, nrows, n_per_row, imatrix); break;
