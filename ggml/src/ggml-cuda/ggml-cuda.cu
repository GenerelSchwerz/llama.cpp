...
static bool ggml_cuda_can_fuse_f32_q8_0_cpy_pair(
    ...
    if (dst0->ne[0] <= 0 || dst0->ne[0] % QK8_0 != 0 || dst0->ne[1] <= 0 ||
            dst0->ne[2] != 1 || dst0->ne[3] != 1) {
        return false;
    }
    if (!ggml_is_contiguous(src0) || !ggml_is_contiguous(src1) ||
            !ggml_is_contiguous(dst0) || !ggml_is_contiguous(dst1)) {
        return false;
    }
    if ((uintptr_t) src0->data % alignof(float) != 0 ||
            (uintptr_t) src1->data % alignof(float) != 0 ||
            (uintptr_t) dst0->data % alignof(block_q8_0) != 0 ||
            (uintptr_t) dst1->data % alignof(block_q8_0) != 0) {
        return false;
    }
