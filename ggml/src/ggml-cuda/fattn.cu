...
static void ggml_cuda_flash_attn_ext_mma_quant_switch_head_size(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    switch (dst->src[0]->ne[0]) {
        ...
        case 256:
            ...
            break;
        case 512:
            if constexpr ((type_K == GGML_TYPE_Q4_0 || type_K == GGML_TYPE_Q4_1 || type_K == GGML_TYPE_Q5_0 || type_K == GGML_TYPE_Q8_0) && type_V == type_K) {
...
static bool ggml_cuda_fattn_native_applies(
    ...
    const bool supported_head_size = Q->ne[0] == 64 || Q->ne[0] == 128 || Q->ne[0] == 256 ||
        (Q->ne[0] == 512 && (K->type == GGML_TYPE_Q4_0 || K->type == GGML_TYPE_Q4_1 || K->type == GGML_TYPE_Q5_0 || K->type == GGML_TYPE_Q8_0) && V->type == K->type &&
