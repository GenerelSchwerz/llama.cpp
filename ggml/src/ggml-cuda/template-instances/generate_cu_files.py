...
SOURCE_FATTN_MMA_QUANT_CASE = "DECL_FATTN_MMA_QUANT_CASE({type}, {head_size}, {head_size}, {ncols1}, {ncols2});\n"
...
for ncols in [8, 16, 32, 64]:
    for ncols2 in [1, 2, 4, 8]:
        if ncols2 > ncols:
            continue
        ncols1 = ncols // ncols2
        with open(f"fattn-mma-quant-instance-ncols1_{ncols1}-ncols2_{ncols2}.cu", "w") as f:
            f.write(SOURCE_FATTN_MMA_QUANT_START)
            for type_name in FATTN_MMA_QUANT_TYPES:
                for head_size in FATTN_MMA_QUANT_HEAD_SIZES:
                    f.write(SOURCE_FATTN_MMA_QUANT_CASE.format(
                        type=type_name, head_size=head_size, ncols1=ncols1, ncols2=ncols2))
            f.write("\n#ifdef GGML_CUDA_FA_ALL_QUANTS\n")
            for type_name in FATTN_MMA_QUANT_TYPES_EXTRA:
                for head_size in FATTN_MMA_QUANT_HEAD_SIZES:
                    f.write(SOURCE_FATTN_MMA_QUANT_CASE.format(
                        type=type_name, head_size=head_size, ncols1=ncols1, ncols2=ncols2))
