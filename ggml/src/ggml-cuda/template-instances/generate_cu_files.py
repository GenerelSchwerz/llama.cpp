...
FATTN_MMA_QUANT_TYPES       = ["GGML_TYPE_Q8_0", "GGML_TYPE_Q4_0", "GGML_TYPE_Q5_0", "GGML_TYPE_Q6_0"]
FATTN_MMA_QUANT_TYPES_EXTRA = ["GGML_TYPE_Q4_1", "GGML_TYPE_Q5_1", "GGML_TYPE_Q6_1",
                               "GGML_TYPE_Q3_0", "GGML_TYPE_Q3_1", "GGML_TYPE_Q2_0S", "GGML_TYPE_Q2_1"]
...
SOURCE_FATTN_MMA_QUANT_CASE = ("DECL_FATTN_MMA_QUANT_CASE({type_k}, {type_v}, "
                               "{head_size}, {head_size}, {ncols1}, {ncols2});\n")

# Ordered K/V pair policy for the quantized-native MMA kernels. This mirrors
# ggml_cuda_fattn_quant_pair_policy() in fattn-mma-quant.cuh and
# ggml_cuda_get_fattn_vec_default_pairs() in ggml/CMakeLists.txt: on the bit
# ladder below, V may sit at K's position or up to two positions lower, never
# higher, and at equal position a _1 K may pair with a _0 V but not the reverse.
# 48 of the 121 ordered pairs over the eleven native types.
FATTN_MMA_QUANT_LADDER = [
    ["GGML_TYPE_Q8_0"],
...
def fattn_mma_quant_rank(type_name):
    for rank, rung in enumerate(FATTN_MMA_QUANT_LADDER):
        if type_name in rung:
            return rank
    raise KeyError(type_name)

def fattn_mma_quant_variant(type_name):
    return FATTN_MMA_QUANT_LADDER[fattn_mma_quant_rank(type_name)].index(type_name)

def fattn_mma_quant_pair(type_k, type_v):
    rank_k = fattn_mma_quant_rank(type_k)
    rank_v = fattn_mma_quant_rank(type_v)
    if not 0 <= rank_v - rank_k <= 2:
        return False
    if rank_k == rank_v:
        return fattn_mma_quant_variant(type_k) <= fattn_mma_quant_variant(type_v)
    return True
    ...
    for ncols2 in [1, 2, 4, 8]:
        ...
        for type_k in FATTN_MMA_QUANT_TYPES + FATTN_MMA_QUANT_TYPES_EXTRA:
            ...
            with open(name, "w") as f:
                ...
                for tier, types_v in (("", FATTN_MMA_QUANT_TYPES),
                                      ("extra", FATTN_MMA_QUANT_TYPES_EXTRA)):
                    cases = [(type_v, head_size)
                             for type_v in types_v
                             if fattn_mma_quant_pair(type_k, type_v)
                             for head_size in FATTN_MMA_QUANT_HEAD_SIZES]
                    if not cases:
                        continue
                    # A pair is in the default tier only if both of its types
                    # are; a default K with an extra V still needs the guard.
                    guard = tier == "extra" and not extra_k
                    if guard:
                        f.write("\n#ifdef GGML_CUDA_FA_ALL_QUANTS\n")
                    for type_v, head_size in cases:
                        f.write(SOURCE_FATTN_MMA_QUANT_CASE.format(
                            type_k=type_k, type_v=type_v, head_size=head_size,
