...
// cache; this loader fills the byte-identical tile straight from the quantized
// cache, so nothing downstream of the load (ldmatrix, mma, softmax, combine)
// changes and no F16 copy has to be materialized.
//
// Bit-identity is the contract, not a tolerance: each type's dequant() below
// must reproduce the arithmetic of the corresponding dequantize_block_*_f16 in
// convert.cu exactly, including the order and width of the half operations. A
// type whose dequant() merely computes the same value in a different order can
// differ in the last bit and is not admissible here.
//
// Every supported block layout is 32 elements wide, so a per-thread run of 16
// contiguous elements is exactly half a block and never straddles a block
// boundary. That is what lets the generic loader below index blocks once per
// row outside the inner loop.

// Per-type block layout and dequantization. Specialize for each admissible
// cache type; the primary template is intentionally undefined so that an
// unsupported type fails to compile rather than silently falling back.
template <ggml_type type>
struct fattn_quant_type_traits;

// block_q8_0 (ggml-common.h) is { ggml_half d; int8_t qs[QK8_0]; }, 34 bytes,
// with element i dequantizing to d*qs[i]. The F16-cast path
// (dequantize_block_q8_0_f16, convert.cu) computes that product as
// __hmul2(make_half2(qs.x, qs.y), __half2half2(d)); dequant() below performs
// the same half2 multiply, so the two paths produce bit-identical tiles.
template <>
struct fattn_quant_type_traits<GGML_TYPE_Q8_0> {
    ...
    // Rows are 34 bytes per 32 elements and therefore only 2-byte aligned, so
    // no wider pointer type is well defined for the row base.
    static constexpr int row_align = 2;

    // Dequantize nelem contiguous elements starting at in-block offset iqs
    // into nelem/2 half2 values.
    template <int nelem>
    static __device__ __forceinline__ void dequant(
            const block_t * const __restrict__ blk, const int iqs, half2 * const __restrict__ vals) {
        __align__(16) int8_t qs[nelem];
        ggml_cuda_memcpy_1<nelem, 2>(qs, blk->qs + iqs);
...
// Whether a cache type has a native MMA tile loader compiled in. Kept as a
// constexpr predicate so that both the host-side route decision and the
// device-side kernel selection ask exactly the same question.
static constexpr __host__ __device__ bool ggml_cuda_fattn_quant_template_type(ggml_type type) {
    return type == GGML_TYPE_Q8_0;
}

// Fill tile_KV[i*stride_tile + k], i < nbatch_fa, k < D2, from the quantized
// rows starting at row0, reading half2 pair k0_h2 + k of each row.
//
// KV         is the base pointer of the K/V head, as a raw byte pointer:
//            quantized rows are not half2-typed and generally not half2
//            aligned, so no wider pointer type is well defined here.
...
static __device__ __forceinline__ void flash_attn_ext_quant_load_tile(
    ...
    // dimension. A run of 16 never straddles a block boundary as long as it
    // starts at a multiple of 16, which the static asserts below enforce for
    ...
    static_assert(qk % nelem_per_thread == 0, "per-thread run straddles quant blocks");
...
#pragma unroll
    for (int i0 = 0; i0 < nbatch_fa; i0 += rows_per_iter) {
        ...
        // The staging array is copied through wider types below, so it is
        // aligned explicitly rather than relying on its element alignment.
        __align__(16) half2 vals[nh2_per_thread];

        if (!oob_check || i < i_sup) {
            const block_t * const blk =
                ((const block_t *) (KV + int64_t(row0 + i)*stride_row)) + ib;

            traits::template dequant<nelem_per_thread>(blk, iqs, vals);
        } else {
#pragma unroll
